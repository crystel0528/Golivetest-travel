...
export default function () {
    ...
    const searchRes = http.post('https://dev-api.travelinsider.co/api/xeni/search-activities', searchPayload, { headers });
    check(searchRes, { 'search success': (r) => r.status === 200 });

    sleep(1);

    // Extract first activity ID from response (adjust based on actual API)
    const activity = searchRes.json().data[0];
    if (!activity) return;

    const activityId = activity.activityId;
    const rateCode = activity.rateCode; // example, adjust to actual field
/*/
